  ...
  <div class="flex flex-col">
    <div class="main-grid">
      <div class="post-header">
        ...
        <h1 v-if="article.articleTitle" class="text-white post-title">
          {{ article.articleTitle }}
        </h1>
        <ob-skeleton
          v-else
          class="text-white uppercase post-title"
          width="100%"
          height="clamp(1.2rem, calc(1rem + 3.5vw), 4rem)" />
        <div class="flex flex-row items-center justify-start mt-8 mb-4">
          <div class="post-footer" v-if="article.nickName">
            <img
              class="cursor-pointer hover:opacity-50"
              v-lazy="article.avatar || ''"
              alt="author avatar"
              @click="handleAuthorClick(article.website)" />
    ...
    <div class="main-grid">
      <div>
        ...
        <div v-else class="block min-h-screen py-16 shadow-xl bg-ob-deep-800 px-14 rounded-2xl">
        ...
        <div class="flex flex-col items-end justify-start my-8 lg:flex-row my-gap">
          <div class="self-stretch w-full h-full mr-0 lg:mr-4" v-if="preArticleCard">
            <SubTitle title="settings.paginator.pre" icon="arrow-left-circle" />
            <ArticleCard class="pre-and-next-article" :data="preArticleCard" />
          </div>
          <div class="self-stretch w-full h-full mt-0" v-if="nextArticleCard">
  ...
  setup() {
    ...
    const fetchArticle = () => {
      ...
      api.getArticeById(reactiveData.articleId).then(({ data }) => {
        ...
        metaStore.setTitle(data.content.articleTitle)
        commonStore.setHeaderImage(data.content.articleCover)
        new Promise((resolve) => {
          data.content.articleContent = markdownToHtml(data.content.articleContent)
        ...
        new Promise((resolve) => {
          data.content.preArticleCard.articleContent = md
            .render(data.content.preArticleCard.articleContent)
            .replace(/<\/?[^>]*>/g, '')
            .replace(/[|]*\n/, '')
            .replace(/&npsp;/gi, '')
        ...
        new Promise((resolve) => {
          data.content.nextArticleCard.articleContent = md
            .render(data.content.nextArticleCard.articleContent)
            .replace(/<\/?[^>]*>/g, '')
            .replace(/[|]*\n/, '')
            .replace(/&npsp;/gi, '')
          resolve(data.content.nextArticleCard)
