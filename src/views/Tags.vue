<template>
  <div class="flex flex-col">
    <div class="post-header">
      ...
      <h1 class="text-white uppercase post-title">{{ t('menu.tags') }}</h1>
    </div>
    <div class="block py-16 shadow-xl bg-ob-deep-800 px-14 rounded-2xl">
      <TagList>
        <template v-if="tags != '' && tags.length > 0">
          <TagItem v-for="tag in tags" :key="tag.id" :id="tag.id" :name="tag.tagName" :count="tag.articleCount" size="xl" />
        </template>
      </TagList>
    </div>
  </div>
