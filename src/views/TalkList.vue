  ...
  <div>
    ...
    <div class="flex flex-col">
      <div class="post-header">
        <h1 class="text-white uppercase post-title">{{ t('titles.talks') }}</h1>
      </div>
      <div class="main-grid">
        <div class="relative space-y-5">
          <div
            class="flex p-4 mb-0 shadow-xl bg-ob-deep-800 lg:p-8 rounded-2xl talk-item"
            v-for="item in talks"
            :key="item.id"
            @click="toTalk(item.id)">
            <Avatar :url="item.avatar"></Avatar>
            <div class="talk-info">
              <div class="text-sm user-nickName">
                {{ item.nickName }}
              </div>
              <div class="time">
