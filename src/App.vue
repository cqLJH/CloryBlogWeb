<template>
  <div id="App-Wrapper" :class="[appWrapperClass, theme]" :style="wrapperStyle">
    <div
      id="App-Container"
      class="px-3 app-container max-w-10/12 lg:max-w-screen-2xl lg:px-8"
  ...
  setup() {
    ...
    const fetchWebsiteConfig = () => {
      api.getWebsiteConfig().then(({ data }) => {
        appStore.viewCount = data.content.viewCount
        appStore.articleCount = data.content.articleCount
        appStore.talkCount = data.content.talkCount
        appStore.categoryCount = data.content.categoryCount
        appStore.tagCount = data.content.tagCount
        appStore.websiteConfig = data.content.websiteConfig
      })
    }
